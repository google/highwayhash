...
class ScalarHighwayTreeHashState {
 public:
  ...
  HH_INLINE ScalarHighwayTreeHashState(const Key& keys) {
  ...
  static HH_INLINE void Add(const Lanes& source, Lanes* dest) {
    for (int lane = 0; lane < kNumLanes; ++lane) {
      (*dest)[lane] += source[lane];
    }
  }

  static HH_INLINE void Xor(const Lanes& op1, const Lanes& op2, Lanes* dest) {
    for (int lane = 0; lane < kNumLanes; ++lane) {
...
#define MASK(v, bytes) ((v) & (0xFFull << ((bytes)*8)))
  ...
  static HH_INLINE void ZipperMergeAndAdd(const uint64 v0, const uint64 v1,
                                          uint64* HH_RESTRICT add0,
                                          uint64* HH_RESTRICT add1) {
...
#undef MASK
  ...
  static HH_INLINE uint64 Rot32(const uint64 x) {
    return (x >> 32) | (x << 32);
  }

  static HH_INLINE void Permute(const Lanes& v, Lanes* permuted) {
...
static HH_INLINE uint64 ScalarHighwayTreeHash(const uint64 (&key)[4],
                                              const char* bytes,
                                              const uint64 size) {
